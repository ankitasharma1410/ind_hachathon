...
public class SepaTransferDocumentBuilderTest3 {
	...
	public void testToXml() throws Exception {
		...
		LocalInstrument2Choice locInst = new LocalInstrument2Choice();
		 ...
		 * "C:\\Users\\hackathon\\hackathon-master\\hackathon-master\\second\\com.hackathon1.demo\\input.txt"
		 * ))) { stream.forEach(s-> s.split(",")); String row =
		 * stream.collect(Collectors.joining(",")); }
		 */

		try (BufferedReader br = new BufferedReader(
				new FileReader(new File("D:\\ING_Info\\ING_Repository\\com.hackathon1.demo\\input.txt")))) {
			for (String line; (line = br.readLine()) != null;) {
				...
				if (row[0].equals("110")) {
					...
					SepaTransferDocumentBuilder2.toXml(data, row);
				} else {
					System.out.println();
					// error
				}
		 ...
		 * //assertTrue(result.contains("test- berweisung")); assertTrue(result.
		 * contains("Loooooong Loooooong Loooooong Loooooong Loooooong Loooooong Loooooong "
		 * )); assertFalse(result.
		 * contains("Loooooong Loooooong Loooooong Loooooong Loooooong Loooooong Loooooong N"
		 * ));
		 */
	}
